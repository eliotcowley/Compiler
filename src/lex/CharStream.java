...
public class CharStream
{
   ...
   /** Character representing addop plus or unary plus. */
   public static final char PLUS = '+';
   
   /** Character representing addop minus or unary minus. */
   public static final char MINUS = '-';
   
   /** Character representing end marker. Have to check if next character is also
    dot, in which case we return a doubledot token. */
   public static final char ENDMARKER = '.';
   
   /** String representing doubledot. */
   ...
   /** Next character to be read in (one-character look-ahead). */
   private char nextChar = 0;
   
   /** Two-character look-ahead. */
   private char nextNextChar = 0;
   
   public int tempLineNumber;
    ...
    * <code>LexicalError</code> wasn't encountered during the
   ...
   public boolean open(String filename)
   {
      try
      {
         ...
         //nextNextChar = getNextNextChar();
         
         skipWhiteSpace();
      }
      catch (IOException | LexicalError ex)
      {
   ...
   /** Prints the current line to standard output. */
   ...
   public char currentChar() throws LexicalError
   {
      // Return characters from the stack if there are any.
       /* why do you pop off the stack immediately? wouldn't you want to keep checking
        * to see if you need to push something on? */
      if (!stack.empty())
      {
         return stack.pop();
      }
      ...
      char ch = currentChar;
       /* lineNumber gets incremented if currentChar is at the end of the line, 
        * so by storing it in a temp var */
      ...
      if (ch != EOF && !valid(ch))
      {
        throw LexicalError.IllegalCharacter(ch, tempLineNumber);
      }
      return ch;
   }
...
//      }
   ...
   /** Return the next char after currentChar for one-character look-ahead. */
   public char getNextChar() throws LexicalError {
      return currentChar;
   }
   
   /** Return the second character after currentChar for two-character look-ahead. */
   public char getNextNextChar() throws LexicalError {
       /* assign temporary variables so we can reset them */
       if ((currentLine != null) && (charIndex >= currentLine.length())) {
           return ' ';
       }
       if (currentLine == null) {
